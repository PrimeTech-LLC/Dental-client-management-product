...
const ChangePasswordScreen: React.FC<{ username: string; onDone: () => void }> = ({ username, onDone }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4" style={{ background: 'linear-gradient(135deg, #0a1628 0%, #0f2044 50%, #1a3260 100%)' }}>
      <div className="w-full max-w-sm space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-2xl border border-black/10 overflow-hidden">
          <div className="px-6 py-4 border-b border-amber-100" style={{ backgroundColor: '#fffbeb' }}>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            <div className="space-y-1.5">
              ...
              <input type="password" required value={current} onChange={e => setCurrent(e.target.value)} disabled={loading}
                ...
                className="w-full px-3 py-2.5 bg-slate-50 border border-slate-300 rounded-lg text-sm focus:outline-none focus:border-blue-800 transition-colors disabled:opacity-60"
            ...
            <button type="submit" disabled={loading}
              className="w-full py-2.5 disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold text-sm rounded-lg shadow-sm transition-colors flex items-center justify-center gap-2 mt-2"
              style={{ backgroundColor: loading ? '#1e40af' : '#1e3a8a' }}
              onMouseEnter={e => { if (!loading) (e.currentTarget as HTMLButtonElement).style.backgroundColor = '#1e40af'; }}
              onMouseLeave={e => { if (!loading) (e.currentTarget as HTMLButtonElement).style.backgroundColor = '#1e3a8a'; }}
            >
              {loading
...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onLogin }) => {
  ...
  // Display name: use fetched clinic name if available, fallback to 'Mehmood Dental Clinic'
  const displayName = clinicName || 'Mehmood Dental Clinic';
  const displayTagline = clinicTagline || 'Management System';

  return (
    <div
      className="min-h-screen flex items-center justify-center p-4"
      style={{ background: 'linear-gradient(135deg, #0a1628 0%, #0f2044 50%, #1a3260 100%)' }}
    >
      <div className="w-full max-w-sm space-y-7">
        ...
        <div className="text-center space-y-3">
          {/* Logo mark */}
          <div
            className="inline-flex items-center justify-center w-16 h-16 rounded-2xl shadow-xl mb-1"
          ...
          {/* MDS abbreviation badge */}
          <div className="flex items-center justify-center gap-2">
            <span
              className="px-2.5 py-0.5 rounded text-white font-black text-sm tracking-widest"
              style={{ backgroundColor: '#1e40af' }}
            >
              MDS
            </span>
